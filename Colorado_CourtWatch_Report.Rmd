...
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, fig.width = 8)
library(tidyverse)
library(tableone)
library(knitr)
library(kableExtra)
...
CreateTableOne(data = table1data)
```
...
cw %>%  
  ...
  mutate(labelq = ifelse(is.na(ChargesCategorized) == TRUE, paste0(n, " Cases Charged"), "")
         ) %>%
  filter(is.na(WhichCourt) == FALSE) %>%
  ...
  geom_text(aes(label= paste0(round(percent), "%"), y = percent+0.5), position = position_dodge(width = .9), size = 3  ) +
  geom_text(aes(label= paste0(labelq ),  y = 0), vjust = 3, size = 3 ) +
